 ...
 * FIXED: domain lopinuze.online, output docs/, fonts URL, ads injected
 */
const fs=require('fs');const path=require('path');
const ads = require('./ads.js');
const S='LOPINUZE';const D='lopinuze.online';const O=path.join(__dirname,'docs');
...
const LANG=`<div class="lang-switcher"><select onchange="if(this.value){window.location.href='https://translate.google.com/translate?hl='+this.value+'&sl=en&u='+encodeURIComponent(window.location.href)}"><option value="">\u{1F310}</option><option value="es">ES</option><option value="fr">FR</option><option value="de">DE</option><option value="it">IT</option><option value="pt">PT</option><option value="ru">RU</option><option value="zh">\u{4E2D}\u{6587}</option><option value="ja">\u{65E5}\u{672C}\u{8A9E}</option><option value="ko">\u{D55C}\u{AD6D}\u{C5B4}</option><option value="ar">\u{0627}\u{0644}\u{0639}\u{0631}\u{0628}\u{064A}\u{0629}</option><option value="hi">\u{0939}\u{093F}\u{0928}\u{094D}\u{0926}\u{0940}</option></select></div>`;
...
function injectAds(html){return html.replace('</head>',adMeta+adHead+'</head>').replace('</body>',adFooter+'</body>');}
...
console.log('\n\u{1F389} Vintage newspaper built! '+String(6+sc+ac)+' pages (ads injected)\n');